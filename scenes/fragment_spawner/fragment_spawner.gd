...
@export var num_fragments: int = 3

@onready var _fragment_resource: PackedScene = preload("res://scenes/fragment/fragment.tscn")


func spawn_fragments(direction: Vector2) -> void:
	for i: int in num_fragments:
		var fragment: Fragment = _fragment_resource.instantiate()
		fragment.fragment_config = fragment_config
		var spawn_angle_offset: float = randf_range(
		...
		var impulse_magnitude: float = randf_range(MIN_IMPULSE_MAGNITUDE, MAX_IMPULSE_MAGNITUDE)
