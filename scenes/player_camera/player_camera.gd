...
const INITIAL_SHAKE_MAGNITUDE: float = 5.0
const SHAKE_DIMINISH_SPEED: float = 200.0
...
func _set_global_position_from_player() -> void:
	...
	var target_y: float = INF

	for lock_area: CameraLockArea in _registed_lock_areas:
		if lock_area.lock_x:
			target_x = lock_area.global_position.x
		if lock_area.lock_y:
			target_y = lock_area.global_position.y

	var target_position_from_player: Vector2 = (
