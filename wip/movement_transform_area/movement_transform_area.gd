...
const SPEED_MULTIPLE: float = 0.8
...
func _get_transformation_matrix() -> Array[Vector2]:
	var angle: float = direction * PI / 4
	var is_cardinal: bool = is_zero_approx(fmod(angle, PI / 2))
	if is_cardinal:
		# Identity matrix
		return [Vector2.RIGHT, Vector2.DOWN]

	var angle_vector: Vector2 = Vector2.from_angle(angle)
