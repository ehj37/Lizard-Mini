...
@export var audio_configs: Array[PositionalAudioConfig]
...
func _ready() -> void:
	for audio_config_type: PositionalAudioConfig.Type in PositionalAudioConfig.Type.values():
		var config_i: int = audio_configs.find_custom(
			func(c: PositionalAudioConfig) -> bool: return c.type == audio_config_type
		)
		assert(
			config_i != -1,
			(
				"Audio config "
				+ PositionalAudioConfig.Type.keys()[audio_config_type]
				+ " must be defined in PositionalAudioManager"
			)
		)

		var config: PositionalAudioConfig = audio_configs[config_i]
...
func play_audio_at(
	global_pos: Vector2, type: PositionalAudioConfig.Type, pitch_multiplier: float = 1.0
) -> int:
	var config: PositionalAudioConfig = _audio_configs_by_type.get(type)
	assert(config != null, "Audio must be defined in PositionalAudioManager")
	...
	if config.limit != -1 && audio_count > config.limit:
		match config.limit_behavior:
			PositionalAudioConfig.LimitBehavior.NOOP:
				return -1
			PositionalAudioConfig.LimitBehavior.REPLACE_OLD:
...
func cancel_audio(type: PositionalAudioConfig.Type, identifier: int = -1) -> void:
	var audio_players: Array = _audio_players_by_type[type]
	if identifier == -1:
